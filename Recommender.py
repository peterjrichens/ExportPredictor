...
train = SliceData.rca('comtrade_2010_2dg.tsv', yr = 2010)
train = train[train.exportShare>0]
train = graphlab.SFrame(train)

# Train Models
factor = graphlab.recommender.factorization_recommender.create(
    train, user_id='fromCode', item_id='cmdCode',target='hasRCA')

cmd_sim1 = graphlab.recommender.item_similarity_recommender.create(
    train, user_id='fromCode', item_id='cmdCode',target='hasRCA', similarity_type='jaccard')

cmd_sim2 = graphlab.recommender.item_similarity_recommender.create(
    train, user_id='fromCode', item_id='cmdCode',target='hasRCA', similarity_type='cosine')

cmd_sim3 = graphlab.recommender.item_similarity_recommender.create(
    train, user_id='fromCode', item_id='cmdCode',target='hasRCA', similarity_type='pearson')
...
def get_recomm(ctry_name, n, models = recommenders):
    ctry_code = int(SliceData.ctry_lookup(ctry_name, code=False))
    for model in models:
        ...
        for i in range(len(recomm)):
            print '%d. %s'  % (i + 1, SliceData.cmd_lookup(str(recomm.cmdCode[i])))
